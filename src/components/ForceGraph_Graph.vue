  ...
  methods: {
    ...
    shouldBeVisible(node) {
      if (node.group === 0 || node.group === 1 || node.group === 2) {
        if (this.Up && node.up === 1) return true;
        if (this.Mid && node.mid === 1) return true;
        if (this.Down && node.down === 1) return true;
      }
      return false;
    },
    updateStatisticsAndEmit() {
      ...
      const sectors = visibleNodes.filter(node => node.group === 0);
      const companies = visibleNodes.filter(node => node.group === 1);
      const products = visibleNodes.filter(node => node.group === 2);
      const ss = visibleLinks.filter(link => link.group === 0);
      const sc = visibleLinks.filter(link => link.group === 1);
      const sp = visibleLinks.filter(link => link.group === 2);
      const cc = visibleLinks.filter(link => link.group === 3);
      const cp = visibleLinks.filter(link => link.group === 4);
      const pp = visibleLinks.filter(link => link.group === 5);
    ...
    updateNodeVisibility() {
      ...
      svg.selectAll("circle")
          ...
          .attr("fill", (d) => {
            if (d.group === 0 && d.up === 1) return "rgb(163,133,166)";
            if (d.group === 0 && d.mid === 1) return "#2aad5c";
            if (d.group === 0 && d.down === 1) return "#ffec4b";
