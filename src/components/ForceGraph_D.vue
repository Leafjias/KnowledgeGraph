...
export default {
  ...
  methods: {
    ...
    updateStatisticsAndEmit() {
      ...
      const visibleNodes = this.nodes.filter(node => !(node.group === 1 && node.date > localParsedTime));
      const visibleLinks = this.links.filter(link => {
        const sourceVisible = visibleNodes.some(node => node.id === link.source.id);
        const targetVisible = visibleNodes.some(node => node.id === link.target.id);
