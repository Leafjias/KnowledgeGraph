...
export default {
  ...
  methods: {
    ...
    updateNodeVisibility() {
      const timeString = this.time; // 假设 this.time 的格式为 "2020-6"
      const [year, month] = timeString.split("-").map(Number);
      ...
      const hiddenNodes = svg.selectAll("circle")
          .filter(d => (d.group === 3 || d.group === 4 || d.group === 5) && (d.date < firstDayOfMonth || d.date >= firstDayOfNextMonth));

      const visibleNodesByGroup = {积极: 0, 消极: 0, 中立: 0};//统计每种情绪的占比


      svg.selectAll("circle")
          .each(d => {
            if ((d.group === 3 || d.group === 4 || d.group === 5) && (d.date >= firstDayOfMonth && d.date < firstDayOfNextMonth)) {
              visibleNodesByGroup[d.group === 3 ? '积极' : d.group === 4 ? '消极' : '中立']++;
            }
          })
          .attr("display", d => {
            return (d.group === 3 || d.group === 4 || d.group === 5) && (d.date < firstDayOfMonth || d.date >= firstDayOfNextMonth) ? "none" : "inline";
          });
      ...
      // 发送自定义事件给父组件，传递可见节点的数量
      this.$emit("visible-nodes-count", visibleNodesByGroup);
      // 获取指定日期范围内的股价信息
      const stockPricesInRange = this.getStockPricesInRange(firstDayOfMonth, firstDayOfNextMonth);

      // 更新 specialPoints 数组
      this.specialPoints = stockPricesInRange;
      console.log(this.specialPoints);
      // 触发自定义事件并传递 specialPoints 数组
      // 在子组件的 updateNodeVisibility 方法中
      console.log('Before emitting event:', stockPricesInRange);
      this.$emit('stock-price-data-updated', stockPricesInRange);
      console.log('After emitting event');

    },

// 添加一个新方法，用于获取指定日期范围内的股价信息
    getStockPricesInRange(startDate, endDate) {
      const filteredStockPrices = Object.entries(this.price)
          .filter(([date]) => {
            const currentDate = new Date(parseInt(date.slice(0, 4)), parseInt(date.slice(4, 6)) - 1, parseInt(date.slice(6, 8)));
    ...
    async loadData() {
      try {
        ...
        this.price = response.data.results.股价信息;
        ...
        for (const item of news) {
          if (item.name === undefined || item.id === undefined) {
            ...
            const timestamp = Date.parse(item.时间);
            ...
            this.nodes.push({
              ...
              date: dateObj,
              infence: item.影响,
              group: item.影响 === "积极" ? 3 : item.影响 === "消极" ? 4 : 5 //结点颜色代号
            })
            //console.log(item.name);
          }
        }
...
// 箭头定义
      ...
      const text = svg.append('g')
          ...
          .text(function (d) {
            ...
            else return d.name.substring(0, 3) + '..';
          })
          .attr("display", d => {
            return d.group === 1 && d.date < this.time ? "none" : "inline";
          })
