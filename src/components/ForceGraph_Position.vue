<template>
  <svg width="1250" height="570" id="svg1"></svg>
</template>
...
export default {
  ...
  methods: {
    ...
    isVisible(node) {
      // 如果结点名与给定名相同或者结点与给定名连接，则显示
      if (node.name === this.name) {
        return true;
      }

      // 根据原始逻辑处理结点显示
      if (node.group === 2 && !this.product) {
        return false;
      }
      if (node.group === 0 && !this.link) {
        return false;
      }
      if (node.group === 1) {
        return false;
    ...
    isConnectedToName(node) {
      // 判断节点是否与给定名称相连接
      return this.links.some(link => (link.source.id === node.id && link.target.name === this.name) || (link.target.id === node.id && link.source.name === this.name));
    },
    ...
    updateNodeVisibility() {
      ...
      svg.selectAll("circle")
          .attr("r", d => {
            return d.name === this.name ? 30 :
                d.group === 0 ? 18 :
                    d.group === 1 ? 14 :
                        d.group === 2 ? 10 :
                            8; // 更改半径大小，如果节点名与 this.name 匹配则半径为 30，否则根据 group 设置半径
          })
          .style("stroke", d => {
            return d.name === this.name ? "red" : "black"; // 更改节点描边颜色，如果节点名与 this.name 匹配则为红色，否则为黑色
          })
          .attr("display", d => {
            if (d.name === this.name || this.isConnectedToName(d)) {
              return "inline";
            }
            if (d.group === 2 && !this.product) {
              return "none";
            }
            if (d.group === 0 && !this.link) {
              return "none";
            }
            if (d.group === 1) {
              return "none";
      ...
      svg.selectAll(".links")
          .selectAll("path")
          .attr("display", d => {
            if (d.source.name === this.name || d.target.name === this.name) {
              return "inline";
            }
            const sourceVisible = this.isVisible(d.source);
            const targetVisible = this.isVisible(d.target);
      ...
      svg.selectAll(".texts")
          .selectAll("text")
          .attr("font-size", d => {
            return d.name === this.name ? 15 :
                d.group === 0 ? 8 :
                    d.group === 1 ? 7 :
                        d.group === 2 ? 5 :
                            4; // 更改半径大小，如果节点名与 this.name 匹配则半径为 30，否则根据 group 设置半径
          })
          .attr("display", d => {
            if (d.name === this.name || this.isConnectedToName(d)) {
              return "inline";
...
// 箭头定义
      ...
      const node = svg.append('g')
          ...
          .attr("display", d => {
            return d.group === 1 || d.group === 2 ? "none" : "inline";
          })
          .call(d3.drag()
      ...
      function getLinkStyle(d) {
        switch (d.type) {
          case '上游':
            return {strokeWidth: 7, strokeDasharray: '5,5', stroke: '#FF6F00'}; // 较粗的橙色虚线
          case '属于':
